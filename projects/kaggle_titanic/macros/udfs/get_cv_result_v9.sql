{% macro get_cv_result_v9() %}
    create or replace function {{ target.schema }}.get_cv_result_v9(
        clf_name text,
        tbl_name text,
...
try:
    ...
    scaler = RobustScaler()
    fe_selector = SelectPercentile(chi2, percentile=90)
    model = make_pipeline(fe_selector,scaler, clf)
