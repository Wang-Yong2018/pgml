...
config({
    "post-hook": [
		"drop index if exists {{model['schema']}}.ix_brin_k1fpi cascade ; create index ix_brin_k1fpi on {{this}} using brin(date,agent, access, project,lang, title_length)",
		"drop index if exists {{model['schema']}}.ix_hash_k1fpi cascade ; create index ix_hash_k1fpi on {{this}} using hash(page_title)",
		],
    })
}}