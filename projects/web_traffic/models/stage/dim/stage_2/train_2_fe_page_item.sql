...
config({
    "post-hook": [
		"drop index if exists {{model['schema']}}.ix_brin_t2fpi cascade ; create index ix_brin_t2fpi on {{this}} using brin(agent, access, project,lang, title_length)",
		"drop index if exists {{model['schema']}}.ix_hash_t2fpi cascade ; create index ix_hash_t2fpi on {{this}} using hash(page_title)",
		],
    })
}}